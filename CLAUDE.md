...
**INITIALIZE**: Map .agents/steering structure at session start (Priority: 1. tree -d, 2. find -type d) to understand available context domains. Do not read files, only capture the folder hierarchy.

**CONTEXT SELECTION**: Before any task, check if .agents/steering contains relevant context:

- Match task keywords to folder names (e.g., "auth", "tools", "templates")
- Read context.md files from matching folders when they would inform your approach

**ALIGNMENT**: .agents/steering defines THIS project's patterns and overrides external knowledge. When it provides guidance, follow it over general best practices.
